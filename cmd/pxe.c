...
char mipibootargs[CONFIG_SYS_CBSIZE]="initrd=/boot/initrd.gz label=ANBERNIC rootwait quiet loglevel=0  console=ttyUSB0,115200n8 console=tty3 console=ttyS2,1500000n8 fbcon=rotate:3 video=HDMI-A-1:1152x1920@60";
char hdmibootargs[CONFIG_SYS_CBSIZE]="initrd=/boot/initrd.gz label=ANBERNIC rootwait quiet loglevel=0  console=ttyUSB0,115200n8 console=tty3 console=ttyS2,1500000n8 fbcon=rotate:3 video=HDMI-A-1:1280x720@60";
#endif
...
#ifdef HDMI_BOOTARGS
			gpio_request(54, "gpio0_a4");
			gpio_direction_input(54);	

			if(gpio_get_value(54)==0)
			{
			   env_set("bootargs", hdmibootargs);
			   printf("append: %s\n", hdmibootargs);  
			}else{
			   env_set("bootargs", mipibootargs); 
			   printf("append: %s\n", mipibootargs); 	
			}
